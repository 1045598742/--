...
```js
如果两个节点一样，他会复用老的，比较儿子的话会把老的儿子和新的儿子作比较，如果新的有儿子老的没儿子，会把新的插入到老的dom中，老的有儿子新的没儿子的话，会把老的儿子删掉，diff算法的核心是比较两个儿子都有的情况下（vue使用的双指针方式），
第一种情况：前面一样，依次往后比，如果最后一项是新增的，就加入到老的最后去。
第二种情况：后面一样，如果第一项是新增的，就从后往前比，发现第一项是新的，然后把这一项加入到老的第一项。
第三种情况：老节点顺序是ABCD，新节点顺序是DABC,头和头不一样，尾和尾不一样，他会去拿新节点头去比老节点的尾，如果一样，就把老节点的D移动到第一位，反之就是结尾和开头比，意思是一样的。
第四种情况：原来的旧节点是ABCD，新节点是CDME,他会用新节点的第一项C，去旧节点里面去找有没有C，如果有就把就节点的C移动到第一项，这时候旧节点变成了CABD,继续往后拿到新节点的第二项D，用D去旧节点找，如果有也一样移动旧节点的D放到第二项，然后新节点到了M，M去旧节点找，发现找不到，就按顺序放到了旧节点的第三项，后面的E也是一样
...
~~~html
如果没有key的话，vue会采用暴力复用的策略，他不管你从列表哪一项删掉了一个，他都认为你删的是最后一个，就会发生渲染错误，所以key尽量不要使用索引（静态列表用key没关系），原来是012，重新渲染后还是01，还是会觉得你是删掉了最后一项。所以最好用每一项数据自己的独特id之类的。
右图的变化比较大，顺序直接是反了，标签还是一样的，没有key的时候他会一个一个比对，原来的A变成D，B变成C等等，重新创建dom渲染消耗比较大，如果有key的话就会进行移动操作，不需要重新创建dom，diff会比较key和标签。
~~~
...
~~~js
vnode是一个对象来描述节点，template模板编译出来的render函数，render函数里面有createlement方法，createlement执行完之后才是vnode。
~~~
...
console.log(vc2.$options.data)//'jw'

//new Vue({data:{}})这种形式可以写成对象，因为他不会被复用
~~~
...
```javascript
Sub.options = mergeOptions(//mixin用到了这个方法（合并）
      Super.options,
      extendOptions
)
...
~~~js
1.vuex是强依赖于vue的，它不能用在其他框架中。我们使用vuex都是Vue.use(Vuex)；肯定在Vuex中有一个install方法。

2.想要所有组件都拿到一个公共的Vuex数据，需要用到vue的mixin混入。使用生命周期的beforeCreate（混入的生命周期会比组件里定义的先执行）,因为肯定会有一个根实例，所以进行一下判断就好，然后组件里面的组件依次拿它对应的父组件的$store,就实现了所有页面公用一个vuex状态，拿的就是Store类里面的数据，下面第一张图这样写的话是可以直接在页面上进行获取的（$store.state.xxx）,但是如果我改变了状态，页面是不会刷新的，因为没有进行响应式处理，所以就用到了使用vue的依赖收集（看下面第二张图），用了一个类的属性访问器 （get state）把new 的vue实例的state反了出去，这样就会成为响应式，数据一变，视图刷新。

~~~

![1585479960675](D:\学习笔记\vue-review\1585479960675.png)

![1585480639110](D:\学习笔记\vue-review\1585480639110.png)

getter的实现

![1585482388570](D:\学习笔记\vue-review\1585482388570.png)

mutation的实现

![1585483051608](D:\学习笔记\vue-review\1585483051608.png)

action部分

![1585485070660](D:\学习笔记\vue-review\1585485070660.png)

递归处理结构

![1585490617889](D:\学习笔记\vue-review\1585490617889.png)

![1585490502845](D:\学习笔记\vue-review\1585490502845.png)

![1585490464883](D:\学习笔记\vue-review\1585490464883.png)
